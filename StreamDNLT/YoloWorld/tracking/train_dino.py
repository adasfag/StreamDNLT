...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default='baseline_base_dino', help='yaml configure file name')
    parser.add_argument('--stage1_model', type=str, default=None, help='stage1 model used to train SPM.')
    parser.add_argument('--save_dir', type=str,default='./work_dir/dinotrack_full_finetune',help='root directory to save checkpoints, logs, and tensorboard')
    parser.add_argument('--mode', type=str, choices=["single", "multiple"], default="single",
                        help="train on single gpu or multiple gpus")
    parser.add_argument('--nproc_per_node', default=1,type=int, help="number of GPUs per node")  # specify when mode is multiple
...
def main():
    ...
    if args.mode == "single":
        ...
        if args.port==-1:
            ...
            args.port=port
        train_cmd = f"torchrun  --nnodes=1 --master_port %d --nproc_per_node %d lib/train/run_training_dino.py " \
