...
class DistributionBasedCrossAttention(nn.Module):
    ...
    def distribute_attn(self, tgt, sim_logit, tgt_mask):
        # target token
        tgt_score = sim_logit.masked_fill(~tgt_mask, -1e20).softmax(-1)#1 1 320
        tgt_token = self.attn_drop_tgt(tgt_score) @ tgt#1 1 768
    ...
    def forward(self, tem, tem_mask, ctx, ctx_mask, cls_token, flag): # TODO: grounding 不需要更新
        src_ = self.query_embed.weight.unsqueeze(0).repeat(ctx.shape[0], 1, 1) # 1 3 768
        ...
        sim_logit = F.normalize(cls_token, dim=-1).unsqueeze(1) @ F.normalize(tgt, dim=-1).transpose(-2,-1) * self.logit_scale.exp()#求解视觉特征和分类特征相似度
        
        # divide context features based on distribution
        tgt_token, bgd_token, dis_token = self.distribute_attn(tgt, sim_logit, tgt_mask)#tgt_token:1 1 768 bgd_token:1 1 768 dis_token：1 1 768
