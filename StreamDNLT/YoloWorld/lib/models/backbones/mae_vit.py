...
class MaskedAutoencoderViT(nn.Module):
    ...
    def patchify(self, z, x):
        B = x.shape[0]
        # embed patches
        z = self.patch_embed(z)#16 64 768 用共享的CNN进行patch 化
        x = self.patch_embed(x)#16 256 768

        # add pos embed w/o cls token
        z = z + self.pos_embed_z #16 64 768
        x = x + self.pos_embed_x #16 256 768
        cls_token = self.cls_token.expand(B, -1, -1)#16 1 768

        embedding = torch.cat((cls_token, z, x), dim=1) #16 321 768
        return embedding
