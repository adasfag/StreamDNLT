...
from lib.train.data import sampler_video as sampler, opencv_loader, processing, LTRLoader
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        ...
        if name == "GOT10K_vottrain":
            datasets.append(Got10k(settings.env.got10k_dir_train, split='vottrain', image_loader=image_loader))#just for debug
        if name == "GOT10K_train_full":
            datasets.append(Got10k(settings.env.got10k_dir, split='train_full', image_loader=image_loader))
        if name == "GOT10K_votval":
            datasets.append(Got10k(settings.env.got10k_dir_train, split='votval', image_loader=image_loader))
        if name == "COCO17":
...
def build_dataloaders(cfg, settings):
    ...
    transform_joint = tfm.Transform(tfm.RandomHorizontalFlip(probability=0.5),
                                    tfm.ToGrayscale(probability=0.05))#随机翻转，随机转化为灰度图
    ...
    output_sz = settings.output_sz #图片尺寸 模板：128；搜索图像：256；检测图像：256
    search_area_factor = settings.search_area_factor

    # Train sampler and loader
    settings.num_template = getattr(cfg.DATA.TEMPLATE, "NUMBER", 1)#模板数量：1
    settings.num_search = getattr(cfg.DATA.SEARCH, "NUMBER", 1)#搜索图像数量：2
    sampler_mode = getattr(cfg.DATA, "SAMPLER_MODE", "causal")#采样模式：任意
    train_score = getattr(cfg.TRAIN, "TRAIN_SCORE", False)#训练分数：为假
    dynamic_cls = getattr(cfg.TRAIN, "DYNAMIC_CLS", False)#动态分类：为真
    gaussian_iou = getattr(cfg.TRAIN, "GAUSSIAN_IOU", 0.3)#对IOU真值进行高斯化：0.7
    center_jitter_factor_grounding = getattr(cfg.DATA.SEARCH, "CENTER_JITTER_GROUNDING", 3.5)#中心检测图像：4.5
    scale_jitter_factor_grounding = getattr(cfg.DATA.SEARCH, "SCALE_JITTER_GROUNDING", 0.5)#放缩检测图像：0.5
...
def get_optimizer_scheduler(net, cfg):
    param_dicts = [
        {"params": [p for n, p in net.named_parameters() if ("backbone" not in n) and p.requires_grad]},#非backbone训练参数
        {
            "params": [p for n, p in net.named_parameters() if ("backbone" in n) and p.requires_grad],#backbone训练参数
            "lr": cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
        },
    ]
    if cfg.TRAIN.OPTIMIZER == "ADAMW":#用adamw作为优化器
    ...
    if cfg.TRAIN.SCHEDULER.TYPE == 'step':
        ...
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, cfg.TRAIN.EPOCH, eta_min=0, last_epoch=-1)#用CosineAnnealingLR作为优化安排了
    else:
        raise ValueError("Unsupported scheduler")
    return optimizer, lr_scheduler


def get_optimizer_scheduler_adapter(net, cfg):
    ...
    param_dicts = [
        {"params": [p for n, p in net.named_parameters() if ("adapter" in n) and p.requires_grad]},#非backbone训练参数
        {
            "params": [p for n, p in net.named_parameters() if ("backbone" in n) and p.requires_grad],#backbone训练参数
            "lr": cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
        },
