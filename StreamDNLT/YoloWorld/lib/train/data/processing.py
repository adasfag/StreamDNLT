...
class TrackProcessing(BaseProcessing):
    ...
    def _get_jittered_box_memory(self, box,random_size,random_pos,random_pos_z):
        """ Jitter the input box
        args:
            box - input bounding box
            mode - string 'template' or 'search' indicating template or search data
        ...
        jittered_size = box[2:4] * random_size
        max_offset = (jittered_size.prod().sqrt() * random_pos)
        jittered_center = box[0:2] + 0.5 * box[2:4] + max_offset * random_pos_z
        ...
        for s in ['template', 'search']:
            ...
            for ele in attn:
                if (ele == 1).all():
                    data['valid'] = False
                    return data
            for ele in attn:#template 打包成8*8 search 打包成16*16 
            ...
            if s == 'search':
                data[s + '_cls'] = prutils.generate_cls_label(data[s + '_anno'], gaussian_iou=self.gaussian_iou, out_size=feat_size, dynamic=self.dynamic_cls) #search_cls是搜索圆
    ...
    def track_process_memory(self, data: TensorDict):#这里为了增加时序一致性，确保相邻帧添加的噪音一致
        """
        args:
            data - The input data, should contain the following fields:
                'template_images', search_images', 'template_anno', 'search_anno'
        ...
        for s in ['template', 'search']:
            ...
            if s in ['template']:
                ...
                random_size=torch.exp(torch.randn(2) * self.scale_jitter_factor[s])
                random_pos=torch.tensor(self.center_jitter_factor[s]).float()
                random_pos_z=(torch.rand(2) - 0.5)
                jittered_anno = [self._get_jittered_box_memory(a,random_size,random_pos,random_pos_z) for a in data[s + '_anno']]
            w, h = torch.stack(jittered_anno, dim=0)[:, 2], torch.stack(jittered_anno, dim=0)[:, 3]
