...
class GroundingAndTrackingSampler(torch.utils.data.Dataset):
    ...
    def sample_vl(self):
        ...
        while not valid:
            ...
            template_frames, template_anno, meta_obj_train = dataset.get_frames(seq_id, template_frame_ids, seq_info_dict)#1139
            search_frames, search_anno, meta_obj_test = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict)#1318 1169
            ...
            if language is None:
                data['valid'] = False
            data = TensorDict({ 'template_images': template_frames,#模板信息
                                'template_anno': template_anno['bbox'],#后台拿到的一个批次是这样 一系列连续的间隔图像帧
                                'search_images': search_frames,
                                'search_anno': search_anno['bbox'],
                                'text': language*self.num_search_frames,
                                'text_mask': mask*self.num_search_frames,
