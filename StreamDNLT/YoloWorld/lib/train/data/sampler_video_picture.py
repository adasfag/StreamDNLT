...
class GroundingAndTrackingSampler(torch.utils.data.Dataset):
    ...
    def sample_vl(self):
        ...
        while not valid:
            ...
            template_images_track=template_images#template 不归一化
            template_images_track=self.inv_normalize(template_images_track)
            template_images_track=(template_images_track*255).clamp(0.0, 255.0)
            ...
            template_frames_input=[]
            template_frames_data_samples=[]
            for index,template_frame_path in enumerate(template_frames):
                ...
                data_info = dict(img_id=0, img_path=template_frame_path, texts=texts,gt_bboxes=np.array(template_anno['bbox'][index]).reshape(1,-1))#图像路径:str 文本:两个list ['prompt']+['']
                ...
                template_frames_input.append(data_info["inputs"])
                template_frames_data_samples.append(data_info["data_samples"])
            template_frames_input=torch.stack(template_frames_input,dim=0)
            ...
            for index,search_frames_path in enumerate(search_frames):
                if ori_language.endswith('.txt'):
                    with open(ori_language) as f:
                        lines = f.readlines()
                    texts = [[t.rstrip('\r\n')] for t in lines] + [[' ']]
                ...
                data_info = dict(img_id=0, img_path=search_frames_path, texts=texts,gt_bboxes=np.array(search_anno['bbox'][index]).reshape(1,-1))#图像路径:str 文本:两个list ['prompt']+['']
    
                
                data_info = self.test_pipeline(data_info)
                search_frames_input.append(data_info["inputs"])
                search_frames_data_samples.append(data_info["data_samples"])
            search_frames_input=torch.stack(search_frames_input,dim=0)
        
            data = TensorDict({ 'template_images': template_images_track,#模板信息
                                'search_images': search_frames_input,
                                'template_anno': torch.stack(template_anno['bbox'],dim=0),#后台拿到的一个批次是这样 一系列连续的间隔图像帧
                                'search_anno': torch.stack(search_anno['bbox'],dim=0),
                                'flag': torch.tensor([[2]]),
                                'ori_language':ori_language,
                                "search_frames_data_samples":search_frames_data_samples,
                                "template_frames_data_samples":template_frames_data_samples
                                })
