...
class BaseTrainer:
    ...
    def train(self, max_epochs, load_latest=False, fail_safe=True, load_previous_ckpt=False, distill=False):
        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):#按步数进行训练了
                    self.epoch = epoch
    ...
    def save_checkpoint(self):
        ...
        if int(os.environ["LOCAL_RANK"]) <= 0:
            if not os.path.exists(directory):
                print(f"directory doesn't exist, create directory and save checkpoint.")
                os.makedirs(directory)
            else:
                print(f'save checkpoint to {directory}')
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False):
        ...
        net = self.actor.net.module if multigpu.is_multi_gpu(self.actor.net) else self.actor.net#网络
        ...
        if checkpoint is None:
            ...
            if checkpoint_list:
                ...
                if int(os.environ["LOCAL_RANK"]) <= 0:
                    print('no matched checkpoint file found')
                return
        ...
        # Load network
        checkpoint_dict = torch.load(checkpoint_path, map_location='cpu')#将检查点文件按照字典形式加载进来，这里面存着模型参数、优化器、参数、和优化安排了
        ...
        ignore_fields.extend(['lr_scheduler', 'constructor', 'net_type', 'actor_type', 'net_info', 'stats'])
        ...
        for key in fields:
            if key in ignore_fields:
                continue
            if key == 'net':
                net.load_state_dict(checkpoint_dict[key], strict=False)#给网络加载模型了
            elif key == 'optimizer':
