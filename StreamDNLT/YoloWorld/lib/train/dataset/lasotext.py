...
class Lasotext(BaseVideoDataset):
    """ LaSOT dataset.
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, vid_ids=None, split=None, data_fraction=None):
        ...
        """
        root = env_settings().lasotext_dir if root is None else root
        super().__init__('LaSOText', root, image_loader)
    ...
    def _build_sequence_list(self, vid_ids=None, split=None):
        sequence_list = sorted([path.split('/')[-1] for path in glob.glob(os.path.join(self.root, '*', '*'))])
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        language ='object' #just for debug
