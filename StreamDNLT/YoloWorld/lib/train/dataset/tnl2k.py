...
class TNL2K(BaseVideoDataset):
    ...
    def _build_sequence_list(self, vid_ids=None, split=None):
        seq_path = glob.glob(os.path.join(self.root, '*/'))
    ...
    def _read_language(self, seq_path):
        ...
        try:
            ...
            print(f'find a path that the language_file is nan, the path is {language_file}') #just for debug
            language='language'
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        language = self._read_language(seq_path)
        anno_frames = {}
    ...
    def _get_frame_path(self, seq_path, frame_id):
        images = sorted(glob.glob(os.path.join(seq_path, 'imgs', '*')))
        return images[frame_id]    # frames start from 1
