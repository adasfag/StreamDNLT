...
class MSCOCOSeq(BaseVideoDataset):
    ...
    def _get_frames_path(self, seq_id):
        path = self.coco_set.loadImgs([self.coco_set.anns[self.sequence_list[seq_id]]['image_id']])[0]['file_name']
        return os.path.join(self.img_pth, path)
    ...
    def get_meta_info(self, seq_id):
        try:
            ...
            object_meta = OrderedDict({'object_class_name': cat_dict_current['name'],
                                       ...
                                       'root_class': None,
                                       'motion_adverb': None,
                                       'language': generate_sentence(cat_dict_current['name'].lower())})
        except:
            object_meta = OrderedDict({'object_class_name': None,
                                       ...
                                       'root_class': None,
                                       'motion_adverb': None,
                                       'language': generate_sentence('object, thing or foreground')})
        return object_meta
    ...
    def get_frames_path(self, seq_id=None, frame_ids=None, anno=None):
        # COCO is an image dataset. Thus we replicate the image denoted by seq_id len(frame_ids) times, and return a
        # list containing these replicated images.
        frame = self._get_frames_path(seq_id)
