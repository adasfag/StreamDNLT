...
class DINOTrackActor(BaseActor):
    ...
    def forward_pass(self, data):
        ...
        data_batch = dict(inputs=data_batch["inputs"],
        ...
        for index,data_sample in enumerate(data_batch["data_samples"]):
            ...
            top_padding=data_sample.pad_param[0]
            left_padding=data_sample.pad_param[2]
            
            
            t1 = h if h > w else w
            t2= hs if hs>ws else ws
            t1=t1/t2
            
            
            
            
            gt_instance.bboxes=gt_bboxes[index].reshape(1,-1)/t1+torch.tensor([[left_padding,top_padding,left_padding,top_padding]],device=gt_bboxes.device)#全图片训练结果
        ...
        data_batch = dict(inputs=data_batch["inputs"],
            data_samples=(data_batch["data_samples"],gt_instances,image_metas,torch.ones(1)),
            mode = 'loss'
            )
