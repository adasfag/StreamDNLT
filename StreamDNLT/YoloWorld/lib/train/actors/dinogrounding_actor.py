...
@registry.ACTORS.register("dinogrounding")
class DINOTrackActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        gt_cls = data['search_cls'].reshape(n*b, hc, wc)  # 16 16 16
        
        # compute losses
        size = data['search_images'].shape[-1]//16
        if MEMORY_SEQENCE:
            loss, status = self.compute_losses_confidence(out_dict, gt_bboxes, gt_cls, self.cont_gt(gt_bboxes, size))
        else:
    ...
    def forward_pass(self, data):
        ...
        template_anno = data['template_anno'].repeat(n, 1, 1).reshape(n*b, -1)#32 4
        search_images = data['search_images'].reshape(n*b, 3, hs, ws)#32 3 256 256
        search_anno = data['search_anno'].reshape(n*b, -1)#32 4
        text = data['text'].reshape(n*b, -1)#32 40
        text_mask = data['text_mask'].reshape(n*b, -1)#32 40
        ori_text=data['ori_language'] #list 8 str
        ori_text = [item for item in ori_text for _ in range(n)]
        text = NestedTensor(text, text_mask)
        template_mask = self.anno2mask(template_anno, wt//16)#32 64
        context_mask = self.anno2mask(search_anno, ws//16, reverse=True)#32 256
        flag = data['flag'].repeat(n, 1, 1).reshape(n*b, -1)#16 1 0:tracking 1:grounding 2:visual
 
        ori_text = [item+' .' for item in ori_text]
        out_dict =self.net(search_images, test=False,captions=ori_text)#仅仅训练grounding
        return out_dict
    ...
    def compute_losses_class_score(self, pred_dict, gt_bbox, gt_cls, gt_cont):
        # Get boxes
        pred_logits = pred_dict['pre_class_score'] # [6,16,900,1] 
        pred_boxes = pred_dict['pred_boxes'] #[6,16,900,4]
        ...
        lvl,bs,num_query,num_channel=pred_logits.shape
        ...
        pred_logits_last_gt=torch.ones_like(pred_logits)
        cls_cost=self.objective['cls_cost'](pred_logits.reshape(-1,1),pred_logits_last_gt.reshape(-1,1))
        cls_cost=cls_cost.reshape(lvl,bs,num_query,num_channel)
        ...
        # 获取真值
        pre_logits_com=torch.gather(pred_logits.squeeze(-1),dim=-1,index=indices)
        ...
        min_indices = cost.argmin(dim=-2)  # 形状为 [6, 16, 1]
        pre_logits_com_neg=torch.gather(pred_logits.squeeze(-1),dim=-1,index=min_indices)
        ...
        for lvl,pred_logit in enumerate(pred_logits):
            pred_box=pred_boxes[lvl]
            pre_class_score=pre_logits_com[lvl]
            pre_class_score_neg=pre_logits_com_neg[lvl]
            pred_boxes_vec = box_cxcywh_to_xyxy(pred_box).view(-1, 4)
            num_queries=1#取1个query
            gt_boxes_vec = box_xywh_to_xyxy(gt_bbox)[:, None, :].repeat((1, num_queries, 1)).view(-1, 4).clamp(min=0.0, max=1.0)  # (B,4) --> (B,1,4) --> (B,N,4)
            try:
                ...
                giou_loss, iou = torch.tensor(0.0).cuda(), torch.tensor(0.0).cuda()
            l1_loss = self.objective['l1'](pred_boxes_vec, gt_boxes_vec)  # (BN,4) (BN,4) 但目标检测 直接按照列数排列了
            gt_class_score=torch.ones_like(pre_class_score)
            class_score_loss=self.objective['cls'](pre_class_score,gt_class_score)
            
            gt_class_score_neg=torch.zeros_like(pre_class_score_neg)
            class_score_loss_neg=self.objective['cls'](pre_class_score_neg,gt_class_score_neg)
            
            
            loss = loss+self.loss_weight['giou'] * giou_loss + self.loss_weight['l1'] * l1_loss+2.0*class_score_loss+2.0*class_score_loss_neg
        # status for log
        status = {"Loss/total": loss,
                    "Loss/giou": giou_loss,
                    "Loss/l1": l1_loss,
                    "Loss/class_score": class_score_loss,
                    "Loss/class_score_neg": class_score_loss_neg
                    }
        return loss, status
    ...
    def compute_losses_confidence(self, pred_dict, gt_bbox, gt_cls, gt_cont):
        ...
        confidence_loss = self.objective['class_score'](confidence_query[:,0], gt_confidence)
        ...
        status = {"Loss/total": loss,
                    ...
                    "Loss/l1": l1_loss,
                    "Loss/cls": cls_loss,
                    "Loss/confidence":confidence_loss
                    }
        return loss, status
