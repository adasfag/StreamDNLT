...
class UVLTrackActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        loss, status = self.compute_losses(out_dict, gt_bboxes, gt_cls, self.cont_gt(gt_bboxes, size)) #loss 一个tensor status 一个字典 各个tensor的loss
    ...
    def forward_pass(self, data):
        ...
        template_images = data['template_images'].repeat(n, 1, 1, 1, 1).reshape(n*b, 3, ht, wt)#16 3 128 128
        template_anno = data['template_anno'].repeat(n, 1, 1).reshape(n*b, -1)#16 4
        search_images = data['search_images'].reshape(n*b, 3, hs, ws)#16 3 256 256
        search_anno = data['search_anno'].reshape(n*b, -1)#16 4
        text = data['text'].reshape(n*b, -1)#16 40
        text_mask = data['text_mask'].reshape(n*b, -1)#16 40
        text = NestedTensor(text, text_mask)
        template_mask = self.anno2mask(template_anno, wt//16)#16 64
        context_mask = self.anno2mask(search_anno, ws//16, reverse=True)#16 256
    ...
    def compute_losses(self, pred_dict, gt_bbox, gt_cls, gt_cont):
        ...
        try:
            giou_loss, iou = self.objective['giou'](pred_boxes_vec, gt_boxes_vec)  # 16 4             16 4
        except:
            giou_loss, iou = torch.tensor(0.0).cuda(), torch.tensor(0.0).cuda()
        # compute l1 loss
        l1_loss = self.objective['l1'](pred_boxes_vec, gt_boxes_vec)  # 16 4                          16    4
        cls_loss = self.objective['cls'](pred_cls, gt_cls)      #16 16 16                             16       16        16
        cont_loss = self.objective['cont'](pred_cont.reshape(-1, 2), gt_cont.reshape(-1))#16*256 2             16*256 1
        # weighted sum
