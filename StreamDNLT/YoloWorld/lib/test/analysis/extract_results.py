...
def calc_seq_err_robust(pred_bb, anno_bb, dataset, target_visible=None):
    ...
    if torch.isnan(pred_bb).any() or (pred_bb[:, 2:] < 0.0).any():
        ...
        pred_bb[:, 2:]=0.0
        pred_bb[torch.isnan(pred_bb)]=0.0
    

    if torch.isnan(anno_bb).any():
        if dataset == 'uav' or dataset == 'itb':
            pass
        else:
    ...
    err_center = calc_err_center(pred_bb, anno_bb)#2788 求框的中心距离的误差
    err_center_normalized = calc_err_center(pred_bb, anno_bb, normalized=True)#2788 归一化后的框的中心距离误差
    err_overlap = calc_iou_overlap(pred_bb, anno_bb)#2788 交并比
...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        # Load anno
        anno_bb = torch.tensor(seq.ground_truth_rect)#2788 4
