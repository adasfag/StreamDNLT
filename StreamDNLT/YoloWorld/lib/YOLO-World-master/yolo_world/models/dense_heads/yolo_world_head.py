...
class YOLOWorldHeadModule(YOLOv8HeadModule):
    ...
    def forward_single(self, img_feat: Tensor, txt_feat: Tensor,
        ...
        b, _, h, w = img_feat.shape#1 256 40 40
        cls_embed = cls_pred(img_feat)#根据图像特征获取类别嵌入 1 768 32 32
        cls_logit = cls_contrast(cls_embed, txt_feat)#对图像特征和文本特征进行对比学习 1 13 32 32
        bbox_dist_preds = reg_pred(img_feat)#根据图像特征进行bbox预测 1 64 32 32
        if self.reg_max > 1:
        ...
        if self.training:
            ...
            return cls_logit, bbox_preds #bbox_preds 得到预测的像素框 非归一化 1 4 32 32 
...
class YOLOWorldHead(YOLOv8Head):
    ...
    def loss(self,img_feats,
        ...
        # Fast version
        loss_inputs = outs + (bboxes_labels,
                              img_metas)
    ...
    def predict(self,
        ...
        outs = self(img_feats, txt_feats)#根据图像特征和文本特征获取多尺度的cls预测和box预测结果了
        predictions = self.predict_by_feat(*outs,
                                           batch_img_metas=batch_img_metas,
                                      rescale=rescale)
        ...
        batch_data_samples[0].saved_cls_and_box=outs
        return predictions
        ...
        for (bboxes, scores, objectness,
            ...
            if cfg.multi_label is False:
                ...
                scores, labels, keep_idxs, _ = filter_scores_and_topk(
                    scores, score_thr, nms_pre)#根据阈值和预测的数量对scores进行筛选
