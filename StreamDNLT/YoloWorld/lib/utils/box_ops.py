...
def box_xywh_to_cxcywh_scale(x, f=1):
    x1, y1, w, h = x.unbind(-1)
    b = [x1 + w/2, y1 + h/2, w*f, h*f]
    return torch.stack(b, dim=-1)
...
def giou_loss_cost(boxes1, boxes2):
    """
...
def clip_box_tensor(boxes: torch.Tensor, H: int, W: int, margin: int = 0) -> torch.Tensor:
    """
    boxes: Tensor of shape [N, 4], format [x, y, w, h]
    Returns: Tensor of shape [N, 4], clipped boxes
    """
    x1 = boxes[:, 0]
    y1 = boxes[:, 1]
    w = boxes[:, 2]
    ...
    x1 = torch.clamp(x1, 0, W - margin)
    x2 = torch.clamp(x2, margin, W)
    y1 = torch.clamp(y1, 0, H - margin)
    y2 = torch.clamp(y2, margin, H)

    # Recalculate width and height after clipping
    w = torch.clamp(x2 - x1, min=margin)
    h = torch.clamp(y2 - y1, min=margin)
...
class GaussWeightedLoss(nn.Module):
    def __init__(self, alpha=2, beta=4, reduction='sum'):
        super().__init__()
        self.alpha = alpha
        self.beta = beta
        self.reduction = reduction
    ...
    def forward(self, pred, gt):
        ...
        if self.reduction == 'sum':
            loss = - (pos_loss + neg_loss) # / pred.nelement()
